...
const Calculators = () => {
  ...
  // Creatina
  const [grams, setGrams] = useState<string>("5");
  const [stock, setStock] = useState<string>("300");
        ...
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          >
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-body font-medium mb-4">
              <Calculator className="w-3.5 h-3.5" /> Ferramentas Horen
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-heading text-xl font-bold text-foreground">Duração da Creatina</h2>
                  <p className="text-xs text-muted-foreground font-body">Estime quanto tempo seu pote dura</p>
                </div>
              </div>

              <div className="space-y-4">
                <div>
                  <Label htmlFor="grams">Dose diária (g)</Label>
                  <Input id="grams" type="number" min="0" step="0.5" value={grams} onChange={(e) => setGrams(e.target.value)} className="mt-1.5" />
                </div>
                <div>
                  <Label htmlFor="stock">Quantidade no pote (g)</Label>
                ...
                {creatinaResult && (
                  <div className="mt-6 p-5 rounded-xl bg-primary/5 border border-primary/20">
                    <p className="text-xs uppercase tracking-wider text-muted-foreground font-body mb-1">Vai durar</p>
                    <p className="font-heading text-3xl font-bold text-primary">{creatinaResult.days} dias</p>
                    <p className="text-sm text-foreground font-body mt-2">
                      Previsão de término: <span className="font-semibold">{creatinaResult.endDate}</span>
                    </p>
                  </div>
                )}
              </div>
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-heading text-xl font-bold text-foreground">Taxa Metabólica Basal</h2>
                  <p className="text-xs text-muted-foreground font-body">Fórmula de Mifflin-St Jeor</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="act">Nível de atividade</Label>
                  <select
                    id="act"
                    value={activity}
                ...
                {tmbResult && (
                  <div className="mt-6 p-5 rounded-xl bg-primary/5 border border-primary/20">
                    <p className="text-xs uppercase tracking-wider text-muted-foreground font-body mb-1">Sua TMB</p>
                    <p className="font-heading text-3xl font-bold text-primary">{tmbResult.tmb} kcal/dia</p>
                    <p className="text-sm text-foreground font-body mt-2">
                      Gasto total estimado: <span className="font-semibold">{tmbResult.gasto} kcal/dia</span>
                    </p>
                  </div>
                )}
              </div>
